...
def criar_title_graf_tab(col):
    ...
    Entrada: "(OBJ)12.\\t(ESTIMULADA) SE A ELEIO PARA SENADOR FOSSE HOJE, EM QUAL DESTES CANDIDATOS VOC VOTARIA? (O ELEITOR PODE ESCOLHER AT DOIS NOMES) - PRIMEIRO VOTO"
    Saída: (ESTIMULADA) EM QUEM VOTARIA PARA SENADOR SE FOSSE HOJE? - 1º VOTO
    
    Entrada: "(ABT) POR QU? POR QU? POR QU?"
    Saída: POR QUÊ?
    
    Entrada: "(OBJ)7.\\t(ESTIMULADA) SE A ELEIO PARA GOVERNADOR FOSSE HOJE, EM QUEM DESTE CANDIDATOS VOC VOTARIA?"
    Saída: (ESTIMULADA) SE A ELEIÇÃO FOSSE HOJE, EM QUEM VOCÊ VOTARIA?
    """
